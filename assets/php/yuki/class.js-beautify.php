<?php
	...
	class JSBeautify {
				...
				switch ($this->tokenType) {
					...
					case 'TK_WORD':
						...
						if ($this->lastType != 'TK_END_BLOCK' && in_array(strtolower($this->tokenText), ['else', 'catch', 'finally'])) {
							...
							if ($this->lastText == 'else') {
								$this->printSpace();
							} else if (in_array($this->lastType, ['TK_START_EXPR', '=', ',', 'TK_WORD', 'return', 'throw']) && $this->tokenText == 'function') {
								$this->printSpace();
							} else if ($this->lastType != 'TK_END_EXPR') {
								...
								if (in_array($this->tokenText, $this->lineStarters) && $this->lastText != ')') { $this->printNewLine(null); }
							}
						...
						if ($this->tokenText == 'if' || $this->tokenText == 'else') { $this->ifLineFlag = true; }
						break;
					...
					case 'TK_OPERATOR':
						...
						if ($this->varLine && $this->tokenText != ',') {
							...
							if ($this->tokenText == ':') { $this->varLine = false; }
						}

						if ($this->varLine && $this->tokenText == ',' && $this->currentMode == 'EXPRESSION') { $this->varLineTainted = false; }
					...
					case 'TK_UNKNOWN':
						if ($this->lastText != $this->tokenText) {
							if ($this->lastType == 'TK_SEMICOLON' || $this->lastType == 'TK_START_BLOCK') {
								$this->printNewLine();
							}
		...
		private function getNextToken(&$parserPos) {
			...
			if ($parserPos >= strlen($this->input)) { return ['', 'TK_EOF']; }
			...
			if ($this->options['preserve_newlines']) {
				if ($newLines > 1) {
					for ($i = 0; $i < 2; $i++) { $this->printNewLine($i == 0); }
				}
			...
			if (strpos($this->wordchar, $c) !== false) {
				if ($parserPos < strlen($this->input)) {
					while (strpos($this->wordchar, $this->getInputChar($parserPos)) !== false) {
						...
						if ($parserPos == strlen($this->input)) { break; }
					}
				}
				...
				if ($wantNewLine && $this->lastType != 'TK_OPERATOR' && !$this->ifLineFlag) { $this->printNewLine(null); }
				return [$c, 'TK_WORD'];
			}
			...
			if ($c == '/') {
				...
				if ($this->getInputChar($parserPos) == '/') {
					...
					$parserPos++;
					if ($wantNewLine) { $this->printNewLine(null); }
				...
				if ($parserPos < strlen($this->input)) {
					if ($sep == '/') {
						...
						while ($esc || $inCharClass || $this->getInputChar($parserPos) != $sep) {
							...
							$parserPos++;
							if ($parserPos >= strlen($this->input)) { return [$resultingString, 'TK_STRING']; }
						}
					} else {
		...
		public function getResult() {
			if ($this->addScriptTags) { $this->output .= '</script>'; }
			return $this->output;
		}
		...
		private function getOutputChar($index) { return substr($this->output, $index, 1); }

		private function getInputChar($index) { return substr($this->input, $index, 1); }
		...
		private function printToken() { $this->output .= $this->tokenText; }
		...
		private function removeIndent() {
			if (strlen($this->output) > 0 && $this->getOutputChar(strlen($this->output) - 1) == $this->indentString) {
				$this->output = substr_replace(
					$this->output, '', strlen($this->output) - 1, 1
				);
			}
		}
